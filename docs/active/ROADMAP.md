...
- [#110](https://github.com/udog21/subsounder/issues/110) Add `products.billing_domains text[]` + harvest existing dogfood — schema migration, then populate from `soundings_log.provider_domain` for every product with ≥1 parsed receipt. Marketing `website` is wrong as a sender hint (Stripe/Paddle relay, custom DKIM domains, billing subdomains) (`reliability`)
...
- [#111](https://github.com/udog21/subsounder/issues/111) Catalog-driven Gmail filter generator — `/api/gmail-filters/[pod_id]` endpoint renders a shape-3 `filters.xml` from the catalog's `billing_domains[]` plus a fixed relay-sender list and one phrase backstop. Onboarding flow offers download-and-import; current keyword rule remains as fallback copy-paste for one release while the new flow is observed (`feature`)
