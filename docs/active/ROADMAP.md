...
- [#117](https://github.com/udog21/subsounder/issues/117) Backfill search-string generator + onboarding flow — server-rendered Gmail and Outlook search strings from the `providers` registry, intersection format `(from:<registry>) AND (subscription/renewal/trial/recurring/billing/invoice/receipt)` with 24-month lookback. Seeds catalog from the user's existing inbox without waiting on renewal cycles. Implements ADR-0006 §11 backfill ingestion mode (`feature`)
