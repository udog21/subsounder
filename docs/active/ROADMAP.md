...
_Last updated: 2026-05-29 (ADR-0006 ingestion-shape refinement). This is the strategic plan. For tactical work units, see [GitHub issues](https://github.com/udog21/subsounder/issues) — each phase's Scope below maps to GH issues._
...
### Ingestion shape for the wedge — recall-first per [ADR-0006](../adr/0006-forwarding-ingestion-filter-shape.md)

*The forwarding rule wedge invitees paste is server-rendered from the `products` registry as a Gmail `filters.xml` import. Shape: provider names (from `provider_name` + `aliases[]`) plus a deliberately broad set of SaaS-flavored generic terms (`subscription`, `renewal`, `trial`, `recurring`, `billing`, `invoice`, `receipt`), minus a global exclusion list of observed-noisy senders. Recall-first: the LLM extractor and matcher are the precision layers, so the filter optimizes for catching plausible signals — including trial confirmations from product subdomains, Class C welcome/ToS/anniversary mails (per [ADR-0004](../adr/0004-silent-provider-signals-classes-and-sonar-bench.md)), and receipts relayed via Stripe/Paddle/Lemonsqueezy. The previously-planned `products.billing_domains[]` substrate (#110) is deferred to Phase 2 / Beyond M2, where it supports the post-cutoff-B closed-whitelist and OAuth-managed shapes. Cutoff A (drop `billing`/`invoice`/`receipt`) and Cutoff B (drop all generic terms) are evidence-triggered, instrumented during M1 via per-keyword-path parser-outcome telemetry — the cutoffs themselves are post-M1.*
...
- [#111](https://github.com/udog21/subsounder/issues/111) (rescoped) Catalog-driven Gmail filter generator — `/api/gmail-filters/[pod_id]` endpoint renders a recall-first `filters.xml` from `provider_name` + `aliases[]` + the fixed generic-term set + the global exclusion list. Includes a `safe_to_filter_alone` (or equivalent) curation flag on `products` for common-word colliders (`Apple`, `Linear`, `Notion`), and the per-keyword-path parser-outcome telemetry that triggers Cutoff A and B (`feature`)
...
- Gmail OAuth ingestion (per [ADR-0006](../adr/0006-forwarding-ingestion-filter-shape.md)) — both candidate shapes are CASA Tier 2 gated and deferred until paid base justifies the review cost: **shape 4** (server-managed filter via `gmail.settings.basic`, no backfill, narrow-scope trust story) and **shape 5** (direct ingest via `gmail.readonly` + Pub/Sub watch, 12-month backfill on connect, broader scope). Alpha survey (Phase 2) discovers which shape — or sequencing of both — to commit to. Microsoft Graph mail-read for Outlook business invitees sits behind the same gate plus tenant admin-consent friction; same deferral applies. Post-cutoff-B closed-whitelist precision uses `products.billing_domains[]` (Phase 2 / Beyond M2 prerequisite — formerly #110).
