-- Prompt v3: trial fields, cadence-vs-date imperative, next_renewal inference,
-- today's-date grounding, merchant/plan separation, cancel_by_at on renewal notices.
-- Fixes gh issue #3.
...
- plan_name = the specific plan or tier within that product. Do NOT echo the billing cadence into plan_name — cadence lives in billing_cadence.
- Examples:
...
- cancel_by_at = next_renewal_at minus 1 day, unless the email states a different deadline.$prompt$,
  ...
  'v3: trial fields, cadence-vs-date imperative, next_renewal inference, today date grounding, merchant/plan separation, cancel_by_at on renewal notices. Fixes gh issue #3.',
  true
)
